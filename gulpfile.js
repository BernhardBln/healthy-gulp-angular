...
pipes.validatedAppScripts = function(failOnErrors) {
    // in production, a version.json is expected. Do not catch any errors here,
    // this should fail if the file hasn't been generated by gradle
    var props = JSON.parse(fs.readFileSync("version.json", "utf8"));

    var pipeline = gulp.src(paths.scripts)
        .pipe(plugins.jshint())
        .pipe(plugins.jshint.reporter('jshint-stylish'));


    if (failOnErrors) {
        pipeline = pipeline.pipe(plugins.jshint.reporter('fail', {verbose: true}));
    }
    ...
    if (failOnErrors) {
        pipeline = pipeline.pipe(plugins.jsvalidate());
    }
    return pipeline
...
pipes.builtAppScriptsDev = function() {
    return pipes.validatedAppScripts(false)
        .pipe(gulp.dest(paths.distDev));
};

pipes.builtAppScriptsTest = function() {
    return pipes.validatedAppScripts(true)
        .pipe(gulp.dest(paths.distScriptsTest));
};
pipes.builtAppScriptsProd = function() {
    return pipes.validatedAppScripts(true /* always fail in prod if there are errors! */)
...
gulp.task('validate-app-scripts', _.partial(pipes.validatedAppScripts, true));
